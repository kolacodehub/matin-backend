...
CMD sh -c "python manage.py shell -c \"from django.db import connection; c = connection.cursor(); c.execute('GRANT ALL ON SCHEMA public TO dev-db-202826;')\" && python manage.py migrate --noinput && gunicorn matin.wsgi:application --bind 0.0.0.0:8000"
